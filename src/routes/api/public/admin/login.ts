  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const validation = await isPasswordValid("admin", password);
        if (!validation.valid) {
          return jsonResponse({ ok: false, error: "Invalid password." }, 401);
        }

        const token = createSessionToken("admin", validation.expiresAt);
        console.info("[auth] admin/login success", {
        ...
        return jsonResponse(
          ...
          200,
          { "Set-Cookie": createSessionCookie("admin", token, validation.expiresAt, request) },
        );
      },
    },
  },
